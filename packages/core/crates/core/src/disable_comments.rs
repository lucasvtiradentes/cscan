...
static DISABLE_FILE_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"//\s*tscanner-disable-file").unwrap());
...
static DISABLE_NEXT_LINE_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"//\s*tscanner-disable-next-line\s+(.+)").unwrap());
...
"#;
    ...
    fn test_disable_line() {
        let source = r#"
const x: any = 5; // tscanner-disable no-any-type
const y: any = 10;
"#;
        let directives = DisableDirectives::from_source(source);
...
"#;
    ...
    fn test_disable_multiple_rules() {
        let source = r#"
// tscanner-disable-next-line no-any-type, no-console-log
const x: any = 5;
console.log(x);
"#;
        let directives = DisableDirectives::from_source(source);
